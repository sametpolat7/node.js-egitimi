...
// Node.js, açık kaynaklı ve platformlar arası bir JavaScript çalışma zamanı ortamıdır. Hemen hemen her tür proje için popüler bir araçtır!

// Node.js, Google Chrome'un çekirdeği olan V8 JavaScript motorunu tarayıcının dışında çalıştırır. Bu, Node.js'nin çok performanslı olmasını sağlar.
...
// Node.js'nin en yaygın Hello World örneği bir web sunucusudur: index.js / Example 1
...
// *** NOT ***
// Q1 : Ama yine de bir adresi olması gerekmez mi? Tamam yerel ip, cihazımın yerel ağdaki adresidir. Ancak localhost:3000 adresinde bir sunucu kurduğumda, cihazım tarafından barındırılan bir sunucu da kurmuş olmuyor muyum? Bu durumda, bu sunucunun (yani cihazımın) tanımlı bir adresi olması gerekmez mi?

// R1 : Evet, doğru söylüyorsunuz. localhost:3000 üzerinde bir sunucu kurduğunuzda, gerçekten de kendi cihazınızda bir sunucu barındırmış olursunuz. Bu durumda, cihazınızın yerel ağ içinde tanımlanmış bir adresi vardır ve bu adres genellikle DHCP aracılığıyla yönlendiriciniz tarafından atanır.

// Ancak, localhost veya 127.0.0.1 kullanarak sunucuya eriştiğinizde, esasen geri döngü arayüzü üzerinden kendi cihazınıza bağlanırsınız. Bu, cihazınızın ağ içindeki yerel IP adresi ihtiyacını atlar.

// Ancak sunucunuza aynı yerel ağdaki diğer cihazlardan erişmek istiyorsanız, localhost yerine cihazınızın yerel IP adresini (192.168.2.48:3000 gibi) kullanmanız gerekir. Bu, ağdaki diğer cihazların cihazınızda barındırılan sunucunuza ulaşmasını sağlar.

// * NOT *
// listen(), belirtilen bağlantı noktasında gelen bağlantıları dinlemeye başlamak için kullanılan bir yöntemdir. Genellikle sunucuyu başlatmak ve gelen istekleri kabul etmeye başlamak için http.createServer() veya app.listen() (Express kullanırken) ile birlikte kullanılır.

// on(), EventEmitter örneklerine olay dinleyicileri eklemek için kullanılan bir yöntemdir. Sunucularla çalışırken, genellikle sunucu tarafından yayılan 'hata' gibi olayları dinlemek için kullanılır. Bu, özellikle başka bir işlem tarafından zaten kullanılmakta olan bir bağlantı noktasında bir sunucu başlatmaya çalışırken EADDRINUSE (adres zaten kullanımda) gibi hataları işlemek için kullanışlıdır.
